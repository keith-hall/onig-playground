    ...
    class OnigRegExp {
        ...
        search(text, startPos = 0) {
            ...
            try {
                ...
                // Convert startPos from UTF-16 string position to UTF-8 byte position
                let utf8StartPos = 0;
                if (startPos > 0) {
                    // Get substring up to startPos and encode it to get byte position
                    const textUpToStart = text.substring(0, startPos);
                    utf8StartPos = new TextEncoder().encode(textUpToStart).length;
                }
                ...
                // Call WASM search function with converted UTF-8 byte position

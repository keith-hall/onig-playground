...
(function(global) {
            ...
            try {
                ...
                const moduleInstance = Onigasm({
                    onRuntimeInitialized() {
                        ...
                        isInitialized = true;
                        // Use safe logging to avoid console binding issues
                        if (typeof console !== 'undefined' && console.log) {
                            try {
                                console.log.call(console, 'Onigasm initialized successfully');
                                // Log available functions for debugging
                                const funcs = Object.keys(moduleInstance).filter(k => k.startsWith('_') && typeof moduleInstance[k] === 'function');
                                console.log.call(console, 'Available WASM functions:', funcs);
                            } catch (e) {
                    ...
                    onAbort(what) {
                        if (typeof console !== 'undefined' && console.error) {
                            try {
                                console.error.call(console, 'Onigasm aborted:', what);
                            } catch (e) {
                                // Ignore console errors
                            }
                        }
                        reject(new Error('Onigasm WASM module aborted: ' + what));
                    }
                });
            } catch (error) {
                if (typeof console !== 'undefined' && console.error) {
                    try {
                        console.error.call(console, 'Onigasm initialization error:', error);
                    } catch (e) {
                        // Ignore console errors
                    }
                }
    ...
    class OnigRegExp {
        ...
        _compile() {
            ...
            try {
                // For testing, let's try a very simple pattern compilation
                // Convert pattern to UTF-8 bytes
                const patternBytes = new TextEncoder().encode(this.pattern);
                const patternPtr = onigasmModule._malloc(patternBytes.length + 1); // +1 for null terminator
                onigasmModule.HEAPU8.set(patternBytes, patternPtr);
                onigasmModule.HEAPU8[patternPtr + patternBytes.length] = 0; // null terminator
                
                // Call WASM compile function with simpler flags
                let flagsNum = 0;
                ...
                // Check result - maybe the convention is different
                if (this.compiledPattern === 0 || this.compiledPattern < 0) {
                    // Check if there's an error
                    let errorCode = 'unknown';
                    if (typeof onigasmModule._getLastError === 'function') {
                        errorCode = onigasmModule._getLastError();
                    }
                    throw new Error(`Regex compilation failed with error code: ${errorCode}`);
                }
                
                // Success - we have a positive handle
